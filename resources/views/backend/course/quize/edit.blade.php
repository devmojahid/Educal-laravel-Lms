...
@section('content')
    ...
    <form id="quizForm" action="{{ route('admin.course.updateQuizData', $quizes->id) }}" method="POST">
        @csrf
        <input type="hidden" name="course_id" value="{{ $courseId }}">
        <input type="hidden" name="quize_id" value="{{ $quizes->id }}">
        <div class="card">
            <div class="card-body">
                <div class="form-group">
                    <label for="quizTitle">Quiz Title</label>
                    <input type="text" name="title" class="form-control" id="quizTitle" value="{{ $quizes->title }}"
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <select name="quiz_type" class="custom-select">
                                <option value="multiple" @if ($quizes->quiz_type == 'multiple') selected @endif>Multiple Choise
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="quiz_duration" class="form-control" id="duration"
                                value="@if ($quizes->quiz_duration) {{ $quizes->quiz_duration }} @endif">
                        </div>
                    </div>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="marks_per_question" class="form-control" id="marks_per_question"
                                placeholder="Enter marks per question" value="{{ $quizes->marks_per_question }}" required>
                        </div>
                    </div>
                </div>
