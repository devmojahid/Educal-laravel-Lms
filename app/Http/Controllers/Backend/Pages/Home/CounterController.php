...
class CounterController extends Controller
{
    ...
    {
        $counterData = SystemSetting::where('key', 'home_counter')->first();
        ...
        return view('backend.pages.home.counter.index', compact('counters'));
    }

    public function update(Request $request) {
        $counter = SystemSetting::where('key', 'home_counter')->first();
        if(!$counter){
            $data = [
                'title' => $request->title,
